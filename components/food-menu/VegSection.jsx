...
const VegSection = ({ heading, foodItems }) => {
  ...
  const dayWiseFood = useMemo(() => {
    return foodItems && foodItems.length > 0 ? foodItems : [
    {
      day: "Monday",
        type:"balanced",
      category: "Non-Veg",
              ...
              currentDayFood[meal] ? (
                ...
                >
                  ...
                  <div className='flex flex-wrap justify-between text-sm text-gray-600 gap-2 mt-auto'>
                    <span className="bg-yellow-100 rounded px-2 py-1">Protein: <b>{currentDayFood[meal].protein || 0}g</b></span>
                    <span className="bg-orange-100 rounded px-2 py-1">Carbs: <b>{currentDayFood[meal].carbs || 0}g</b></span>
                    <span className="bg-red-100 rounded px-2 py-1">Fats: <b>{currentDayFood[meal].fats || 0}g</b></span>
                    <span className="bg-green-100 rounded px-2 py-1">Calories: <b>{currentDayFood[meal].calories || 0}</b></span>
                  </div>
                </motion.div>
              ) : null
            )}
